    ...
    MongoClient.connect(url, function(err, db) {
        ...
        autoIncrement.getNextSequence(dbo, doccollet, function(err, autoIndex) {
            ...
            collection.insert({
                ...
                "password": pass,
                "twoFA": twoFA,
                "twofaEnable": twaEnable,
                "firstLogin": offirstlogin
            });
            db.close();
            if (err == null) callback(null, "OK");
            return;
...
obj.insertAssets = function(_password, _ownerID) {
    if (!_ownerID || !_password) return;
    MongoClient.connect(url, function(err, db) {
        var dboC = db.db("asset");
        var doccolletC = "coins";
        var dboT = db.db("asset");
        var doccolletT = "tokens";

        coinFunc.coinbase.CreateAsset(_password, function(err, asRes) {
            if (err == null) {
                autoIncrement.getNextSequence(dboC, doccolletC, function(err, autoIndex) {
                    var collectionC = dboC.collection(doccolletC);
                    collectionC.insert({
                        "coinId": autoIndex,
                        "coinName": "FEN",
                        "coinSymbolId": 1,
                ...
                autoIncrement.getNextSequence(dboT, doccolletT, function(err, autoIndex) {
                    var collectionT = dboT.collection(doccolletT);
                    collectionT.insert({
                        "tokenId": autoIndex,
                        "tokenName": "2COIN",
                        "coinSymbolId": 2,
