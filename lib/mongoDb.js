...
obj.findSpenderAddress = function(userid, callback) {
    console.log(" HELLO COME TO findSpenderAddress " + userid + "  ");
    userid = 11;
    if (!userid) return;
    MongoClient.connect(url, function(err, db) {
        ...
        dbo.collection("coins").find(query).toArray(function(err, result) {
            ...
            var promiseResolve = [];
            coinAddr = result[0].coinAddr;
            ownerid = result[0].ownerid;
            coinName = result[0].coinName;
...
obj.findApprovegasAddress = function(userid, callback) {
...
obj.PairChkCoins = function(userid, senderaddr, docallback) {
    // console.log ( " HELLO COME TO PairChkCoins " + userid + "  " + senderaddr ) ;
    if (!userid) return;
    ...
    MongoClient.connect(url, function(err, db) {
        ...
        dbo.collection("coins").find(query).toArray(function(err, result) {
            ...
            if (result.length == 0) {
                console.log("Error : nodata found ");
                docallback("Error nodata :" + err, null);
                return [];
                if (err) return err;
            };
            var promiseResolve = [];
            ...
            coinFunc.coinbase.getCoinBalance(coinAddr, function(err, res) { //
                if (err == null) {
                    ...
                    console.log("Error : " + err);
                    docallback("Error " + err, null);
                    return true;
                } /* End if err */
                docallback(null, promiseResolve);
                return true;
            }); // End coin Func
            // callback ( null , result  ) ;
            return;
...
obj.PairChkTokens = function(userid, senderaddr, contractid, callback) {
    //  console.log ( " HELLO COME TO SENDTOKEN " + destaddr + "  " + destaddr ) ;
    if (!userid) return;
    MongoClient.connect(url, function(err, db) {
        var dbo = db.db("asset");
        ...
        dbo.collection("tokens").find(query).toArray(function(err, result) {
            ...
            if (result.length == 0) {
                console.log("Error : nodata found ");
                callback("Error nodata " + err, null);
                return [];
            };
            var promiseResolve = [];
            tokenAddr = result[0].tokenAddr;
            ...
            coinFunc.coinbase.getTokenBalance(tokenAddr, contractAddr, function(err, res) { //
                if (err == null) {
                    ...
                    promiseResolve.push({
                        ...
                        "contractAddr": contractAddr,
                        "cryptkey": cryptkey,
                        "tokenBalance": "SYSTEM connection blockchain has error "
                    });
                    callback( null  , promiseResolve );
                    return false ; 
                    //   console.log ( "Error : " + err ) ;
