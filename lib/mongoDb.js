...
obj.insertReg = function(uname, fname, lname, email, birth, pass, twoFA, twofaEnable, callback) {
...
obj.EnableTwoFA = function(userid, callback) {
    MongoClient.connect(url, function(err, db) {
            ...
            var dbo = db.db("accounts");
            dbo.collection("registor").update({ "userid": userid }, { $set: { "twofaEnable": true } }),
                (function(err, result) {
                        console.log(" UPDATE ---> " + JSON.stringify(result, null, '\t'));
                        if (err) {
                            console.log("ERROR  : " + err);
